...
DefectLoop 由 `code-defect-scanner` 和 `defectloop-code-fixer` 两个可独立安装、可串联使用的 Skill 组成。它把代码缺陷处理拆分为只读发现、结构化交接、授权修复和修复后复检，既保留安全边界，也形成完整开发工作流。
...
| `defectloop-code-fixer` | 显式授权后写入 | 校验报告新鲜度、分析根因、执行最小修复、报告真实验证结果 | [查看 README](defectloop-code-fixer/README.md) |
...
- [`packages/defectloop-code-fixer.zip`](packages/defectloop-code-fixer.zip)
...
```text
使用 $defectloop-code-fixer 根据 defect-report.json 修复已确认缺陷，并运行可用验证。
```
